...
### Fixed
- A cloud routine committed as "Claude", the machine's own git identity, and its pull requests
  ended with a "Generated by" footer. The crew now always commits with the name and email in
  `authorship` (required for routines, CI and servers, filled in by `init`), checks the result,
  and reads every pull request back to remove any footer its tools add.
