...
- Add the board's matching labels to every card you file: one severity (`Critical`, `High`,
  ...
  has them. On a board whose labels have no names yet, attach them by colour: red Critical,
  orange High, yellow Medium, green Low, purple Bug, blue Feature. Never create labels in a run.
